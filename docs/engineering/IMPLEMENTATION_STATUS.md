...
| Desktop Beta Workflow Corrections | CANDIDATE READY / INDEPENDENT ACCEPTANCE PENDING | `V2B_DESKTOP_BETA_WORKFLOW_CORRECTIONS.md`; schema 8 explicit classifications, fresh-only ordinary starter buckets, parent-first child chooser, allocation-ledger Overview rollups, separate income/transfer/debt handling, local-clock month hydration, and a separated Split/Allocation action group. 173 automated tests pass; the founder A–F matrix is recorded PASS and direct/DMG synthetic packaged checks pass. This remains a candidate until independent acceptance; signing/notarization remains blocked. |
